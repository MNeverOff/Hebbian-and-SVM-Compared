...
if (hebb)
    ...
    HTe(1+sdi:1+sdi,I) = length(intersect(pr,ac));
    HTe(2+sdi:2+sdi,I) = length(intersect(pr,nAc));
    HTe(3+sdi:3+sdi,I) = length(intersect(nPr,ac));
    HTe(4+sdi:4+sdi,I) = length(intersect(nPr,nAc));
    % fprintf('Program paused. Press enter to continue.\n');
    % pause;
end